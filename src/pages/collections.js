/**
 * Collections Page - Displays a grid of collections with search and sort
 */
...
let landscapeByUser = {};
let collectionsPageIsActive = false;
...
export async function loadCollectionsData() {
  try {
    // Cache: if we already computed collectionsData once, reuse it.
    ...
    collectionsData = Array.from(userMap.values()).map(user => ({
      ...user,
    ...
    for (const user of collectionsData) {
      user.previewPhotos = pickPreviewPhotos(user.userKey);
    }
    
    // Load user statistics (local photo count, total size, last modified)
    await loadUserStats(collectionsData);
...
export async function renderCollectionsPage() {
  ...
  const users = await loadCollectionsData();
  ...
  const collectionsPage = container.querySelector('.collections-page');
  if (collectionsPage) {
    collectionsPage.classList.add('page-transitioning');
    // Force a reflow to ensure the class is applied before any other operations
    collectionsPage.offsetHeight;
  }
  ...
  if (window.__collectionsPageWheelHandler) {
    document.body.removeEventListener('wheel', window.__collectionsPageWheelHandler, { passive: true });
  }
  // Store handler reference for cleanup
  window.__collectionsPageWheelHandler = handleBodyWheel;
...
/**
 * Cleanup (called when leaving collections view).
 * Keeps it lightweight: stop treating page as active and remove global wheel handler.
 */
export function cleanupCollectionsPage() {
  collectionsPageIsActive = false;
  if (window.__collectionsPageWheelHandler) {
    document.body.removeEventListener('wheel', window.__collectionsPageWheelHandler, { passive: true, capture: true });
    window.__collectionsPageWheelHandler = null;
  }
}
...
function setupCollectionsPageHandlers() {
  ...
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      filterAndRenderCollections(e.target.value);
    });
  }
  ...
  if (sortSelect) {
    sortSelect.addEventListener('change', (e) => {
      ...
      filterAndRenderCollections(searchInput?.value || '');
    });
  }
}

function filterAndRenderCollections(searchQuery = '') {
  const query = searchQuery.toLowerCase().trim();
  
  if (query === '') {
    filteredCollections = [...collectionsData];
  } else {
    filteredCollections = collectionsData.filter(user => 
  ...
  filteredCollections.sort(sortFn);
...
function renderCollectionsGrid() {
  ...
  if (filteredCollections.length === 0) {
    grid.innerHTML = '<div class="collections-empty">No collections found</div>';
    return;
  }
  
  grid.innerHTML = filteredCollections.map(user => createUserCard(user)).join('');
