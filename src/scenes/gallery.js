/**
 * Gallery Scene - All gallery logic in one file
 * Handles data fetching, layout generation, camera, rendering, and interactions
 * 
 * ✅ CHECKPOINT: Basic Gallery (git tag: basic-gallery)
...
// US States list (same as in main.js - duplicated for gallery.js)
const US_STATES_GALLERY = {
...
let locationMapGallery = null;

// Map tag to geo label (same logic as in main.js)
function mapTagToGeoLabelGallery(tag) {
  if (!tag) return null;
  ...
  if (US_STATES_GALLERY[normalized]) {
    return US_STATES_GALLERY[normalized];
  }
  
  // Check countries
  if (COUNTRIES_GALLERY[normalized]) {
    return COUNTRIES_GALLERY[normalized];
  }
  
  // Check location map
  if (locationMapGallery && locationMapGallery[normalized]) {
    return locationMapGallery[normalized];
  }
...
export class GalleryScene {
  constructor(canvas) {
    ...
    // Album View state
    this.viewMode = 'gallery'; // 'gallery' | 'album'
  ...
  setupEventHandlers() {
    ...
    this.canvas.addEventListener('wheel', (e) => {
      ...
      if (isCollapseMode) {
        ...
        const delta = isMouseLike
          ? (sign > 0 ? 1.30 : 0.77)  // ~30% zoom in, ~23% zoom out per mouse tick
    ...
    this.canvas.addEventListener('click', (e) => {
      ...
      if (this.viewMode !== 'gallery' || this.transition.active || this.exitTransitionActive) {
        return;
      }
  ...
  enterAlbumMode(photoId) {
    ...
    // Keep viewMode as 'gallery' during transition
  ...
  exitAlbumModeImmediate() {
    ...
    if (this.fromUserAlbums && this.userAlbumsUsername) {
      ...
      // Only set viewMode to gallery if we're not navigating away
  ...
  exitAlbumMode() {
    ...
    // If user clicked on username (go to user page), run transition then navigate — check before fromIndex
  ...
  clampCameraToBounds() {
    ...
    if (this.filtersActive() && !this.filteredLayoutBounds) {      return;
    }
    ...
    const visible = this.camera.getVisibleBounds();    const visibleWidth = visible.right - visible.left;
  ...
  updateZoom() {
    ...
    if (isCollapseMode) {
      ...
      // Normal easing for non-collapse mode
   ...
   */
  computeFocusLayout(visibleTiles, visibleCount, collapseTargetX, collapseTargetY) {    if (visibleTiles.length === 0) {
      this.focusPos.clear();
  ...
  updateCenterDateLabel(centerInfo) {
    ...
    if (this.viewMode !== 'gallery' || this.exitTransitionActive) {
      this.centerDateEl.style.display = 'none';
      return;
    }
  ...
  render() {
    ...
    if (this.exitTransitionActive) {
      ...
      const navigatingToUser = !!this.navigateToUserAfterExit;
      
      if (this.exitStartRect && this.overlayImg) {
        if (navigatingToUser) {
          // Fade out only: keep position/size at start rect, animate opacity 1 -> 0
          this.overlayImg.style.left = `${this.exitStartRect.x}px`;
          this.overlayImg.style.top = `${this.exitStartRect.y}px`;
          this.overlayImg.style.width = `${this.exitStartRect.w}px`;
          this.overlayImg.style.height = `${this.exitStartRect.h}px`;
          this.overlayImg.style.opacity = (1 - ease).toString();
        } else if (this.exitEndRect) {
          // Fly back to tile: interpolate overlay position
          const currentRect = {
      ...
      if (t >= 1) {
        ...
        if (this.fromUserAlbums && this.userAlbumsUsername) {
          ...
          // viewMode will be set to 'gallery' inside requestAnimationFrame above,
    ...
    } else if (this.viewMode === 'gallery' && !this.exitTransitionActive) {
    ...
    if (this.camera.zoom < dynamicMinZoom && !allowZoomOutWhenFiltered) {      this.camera.zoom = dynamicMinZoom;
      this.targetZoom = dynamicMinZoom;
    }
    ...
    if (!this.forceRenderOnce &&
        ...
        !imagesStillLoading &&
        this.viewMode === 'gallery' && 
        this.frameCount > 1) {      // Skip rendering this frame - nothing changed and no images loading
      return;
    }
    ...
    // OPTIMIZATION: When no filters are active, skip iteration and use all tiles directly    let checkedCount = 0;
    let visibleCount = 0;
    ...
    if (hasActiveFilter !== this.lastFilterActive) {
      filterChanged = true;
      this.lastFilterActive = hasActiveFilter;      // Mark filter change time for faster image loading
      ...
      if (filterChanged) {        // Mark filter change time for faster image loading
        this.filterChangeTime = performance.now();
        // Add all visible tiles to queue immediately after filter change
        this.addAllVisibleTilesToQueue();
      ...
      if (this.lastFilterActive) {
        filterChanged = true;
        this.filterChangeTime = performance.now();        // Add all visible tiles to queue immediately after filter clear
        this.addAllVisibleTilesToQueue();      }
    ...
    if (panningActive) {
      ...
      collapseTargetY = this.frozenCollapseTargetY;    } else if (isZoomedInWithCenterText) {
    ...
    if (isCollapseActive && visibleTiles.length > 0) {
      ...
      const useFocusBounds = this.focusAnchorX !== null && this.focusAnchorY !== null && this.focusOffsets && this.focusOffsets.size > 0;
      for (const tile of visibleTiles) {
        let pos = finalPositions.get(tile.id);
        if (useFocusBounds) {
          const offset = this.focusOffsets.get(tile.id);
          if (offset) {
            pos = {
      ...
      if (Number.isFinite(minX)) {
        ...
        this.filteredLayoutBounds = { minX, maxX, minY, maxY };      } else {
    ...
    if (isCollapseActive && visibleTiles.length > 0) {
      ...
      if (shouldRecenter) {
        ...
        const centerY = useBoundsCenter ? (this.filteredLayoutBounds.minY + this.filteredLayoutBounds.maxY) / 2 : (this.focusAnchorY !== null ? this.focusAnchorY : (this.filteredLayoutBounds ? (this.filteredLayoutBounds.minY + this.filteredLayoutBounds.maxY) / 2 : this.camera.y));        this.camera.x = centerX;
    ...
    if (this.viewMode === 'gallery' && !this.transition.active && isCanvasVisible) {
